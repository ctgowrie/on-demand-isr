...
async function getInstallation(token: string) {
  ...
  return installations.find((i: any) => i.account.login === 'ctgowrie');
}
...
export async function fetchIssueAndRepoData() {
  const [issues, repoDetails] = await Promise.all([
    fetchGitHub('/repos/ctgowrie/on-demand-isr/issues', accessToken),
    fetchGitHub('/repos/ctgowrie/on-demand-isr', accessToken),
  ]);
...
export async function fetchIssuePageData(id: string) {
  const [issue, comments, repoDetails] = await Promise.all([
    fetchGitHub(`/repos/ctgowrie/on-demand-isr/issues/${id}`, accessToken),
    fetchGitHub(
      `/repos/ctgowrie/on-demand-isr/issues/${id}/comments`,
      accessToken
    ),
    fetchGitHub('/repos/ctgowrie/on-demand-isr', accessToken),
  ]);
